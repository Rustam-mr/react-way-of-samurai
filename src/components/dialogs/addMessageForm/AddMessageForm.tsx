...
import { GetStringKeys, Textarea } from "../../common/formsControls/FormsControl"
...
type NewFormValuesKeysType = GetStringKeys<NewMessageFormType>

const AddMessageForm: React.FC<ownProps> = (props) => {
    ...
    const { register, handleSubmit, formState: { isValid } } = useForm<NewMessageFormType>({
        mode: 'onChange' // Валидация при каждом изменении, чтобы isValid обновлялся
    })

    const { ref: newMessageBodyRef, ...newMessageBodyRest } = register<NewFormValuesKeysType>("newPostText", { 
    ...
    return (
        ...
        <form onSubmit={handleSubmit(props.onSubmit)} className={s.addMessageForm}>
